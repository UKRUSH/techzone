...
const PCBuilder = () => {
  ...
  const addBuildToCart = async () => {
    ...
    try {
      // Add all components to cart with proper error handling
      const results = await Promise.allSettled(
        components.map(component => addToCart(component.id, 1))
      );
      ...
      if (successful.length > 0) {
        toast.success(`${successful.length} component(s) added to cart!`);
      }
      
      if (failed.length > 0) {
        toast.error(`Failed to add ${failed.length} component(s) to cart`);
      }
    } catch (error) {
